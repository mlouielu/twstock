...
`twstock` 支援興櫃股票的歷史與即時行情查詢，但由於櫃買中心（TPEx）興櫃市場採推薦證券商議價交易，與一般上市上櫃股票不同，其行情資料具有以下特性：

*   **無最佳五檔**：因興櫃無集中市場撮合，即時行情中的最佳五檔買賣價量（`best_bid_*` / `best_ask_*`）是以排名前 5 個最優推薦證券商的個別申報報價代替。
*   **無開盤價、收盤價（歷史行情）**：因官方無提供開、收盤價，歷史行情中的開盤價與收盤價均分別以當日之「日成交加權平均價」代替。
*   **無開盤價、收盤價（即時行情）**：因官方無提供開、收盤價，即時行情中的開盤價以當日之「加權平均價」代替，收盤價（`latest_trade_price`）以「最近成交價」代替。
