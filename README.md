...
`twstock` 支援興櫃股票的歷史與即時行情查詢，但由於櫃買中心（TPEx）興櫃市場採推薦證券商議價交易，與一般上市上櫃股票在交易機制與資料格式上有所不同，其行情資料具有以下特性：

*   **歷史行情 (`twstock.Stock`)**：
    1.  **收盤價 (`close`)**：以官方每日公布之「日成交加權平均價」作為收盤價（所有看盤軟體與 K 線繪製的標準作法）。
    2.  **開盤價 (`open`)**：官方並無提供開盤價，為確保技術指標（如均線、四大買賣點 `BestFourPoint`）能正常進行運算不致出錯，開盤價亦**自動對齊為當日「加權平均價」**。
*   **即時行情 (`twstock.realtime`)**：
    1.  **開盤價 (`open`)**：固定為 `None`。
    2.  **公司全名 (`fullname`)**：藉由櫃買中心 OpenAPI 建立本機對照表，自動解析並填入完整公司全名（如「富味鄉食品股份有限公司」）。
    3.  **最佳五檔**：興櫃股票無集中市場撮合，故最佳五檔買賣價量（`best_bid_*` / `best_ask_*`）回傳的並非市場委託單，而是**排名前 5 個最優推薦證券商的個別申報報價**。
